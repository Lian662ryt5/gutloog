...
function renderFoodPreview(){
  ...
  foodPreview.innerHTML = `
    ${pendingFood.image ? `<img src="${escapeHtml(pendingFood.image)}" alt="" decoding="async">` : `<div class="food-icon">🍽️</div>`}
    <div class="fp-body">
      ${notFound
  ...
  entries.forEach(e=>{
    ...
    if(e.kind === 'food'){
      html += `<div class="log-entry">
        ${e.foodImage ? `<div class="food-icon"><img src="${escapeHtml(e.foodImage)}" alt="" loading="lazy" decoding="async"></div>` : `<div class="food-icon">🍽️</div>`}
        <div class="log-body">
